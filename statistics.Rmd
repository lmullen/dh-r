...
```{r ratios_of_babynames, tidy=FALSE}
library(tidyr)
...
```{r plot_madison_linear_regression}
ratios %>% filter(name == "Madison") %>%
...
```{r test_names_filter, warning=FALSE}
...
```{r plot_linear_regression_five_names, warning=FALSE}
...
```{r model_all_the_names, warning=FALSE}
